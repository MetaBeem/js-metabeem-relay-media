...
describe( 'RateLimit', () =>
{
	...
	{
		const searchLimitPerMinute = 15;
		it( `it should limit ${ searchLimitPerMinute } times searching per minute`, async () =>
		{
			const pfnSendRequestSearch = async () =>
			{
			...
			for ( let i = 0; i < searchLimitPerMinute; i ++ )
			{
				await pfnSendRequestSearch();

				//	wait for a while
				await TestUtil.sleep( 100 );
			}

			//	send the 31st request
			try
			{
				await pfnSendRequestSearch( savedPosts[ 0 ] );
			}
			catch ( err )
			{
				//console.log( err )
				...
				expect( err ).toContain( `Too many requests, please try again later!(searching)` );
			}
